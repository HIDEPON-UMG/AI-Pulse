...
この記事が entity_context（name が指す企業・製品・技術）の話題かを最初に判定します。**同名異義語で別物の記事を取り違えない**ことが最優先です。

- is_relevant: 記事の**主題**が entity_context そのもの、またはその明確な動向（リリース / 資金調達 / 価格 / M&A / 性能・ベンチマーク / 規制 / 事業展開）なら true。次のいずれかなら false:
  - **同名異義の無関係記事**（例: "Runway" = 空港の滑走路 / ファッションの "Rent the Runway"、"Figure" = 図表・人物、"Composer" = 作曲家、"Cosmos" = 宇宙・化粧品、"Devin" = 人名）。
  - entity が記事の主題でなく、別の企業・製品の話に**名前が出てくるだけ**（脇役）。
  - 生成 AI・AI 技術と**無関係な業界**（ファッション / 航空 / スポーツ / 一般政治 等）の話題。
  - 迷ったら「本文の主題が entity_context の AI 製品・技術そのものか」で判断する。
- relevance_reason: is_relevant の判定根拠を日本語 15〜40 字で簡潔に（例: "Runway Gen-4 動画モデル公開が主題" / "空港の滑走路の記事で AI と無関係"）。

is_relevant=false と判断した場合でも schema 厳守のため summary 以降は埋めますが、その精度は問いません（後段で破棄されます）。
