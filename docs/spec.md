...
日次・週次・認証更新のScheduled Taskは、Task Actionを実体`pythonw.exe`とversioned launcherへ固定し、PowerShell childへ`CREATE_NO_WINDOW`を付与する。このサブ端末では既存Disabled状態を保存し、移行処理がbatch実行を開始してはならない。
...
| Events | 782 events | 主課題は「初期記事不足」ではなく、継続収集、品質維持、公開鮮度、entity 間の偏り補正である。 |
...
| Localization | `headline_ja` exists for 756 / 782 events | 全 event 完了済みとは扱わず、欠落分は品質改善 backlog とする。 |
| Visual assets | `thumb` exists for 763 / 782 events | UI 表示の欠落 fallback と補完方針を維持する。 |
...
| Why | 生成AI領域はモデル、開発ツール、物理AI、メディア生成、エージェント、インフラ、政策が同時に動き、単発記事だけでは変化の意味を追いにくい。実務者は短時間で重要度、影響、プレイヤー間の関係を把握したい。既に 782 events あるため、単純増量より、偏り、鮮度、根拠、継続運用の品質が重要である。 |
| What | `data/entities.jsonl` の 32 entity カルテ、`data/events.jsonl` の 782 event、日次/週次バッチ、品質監査、GitHub Pages 公開サイト、BuzzPost / Repo Radar などの周辺ビューを提供する。 |
