...
| ③ フィジカルAI | ⬢ | `0.78 0.145 295` → `#BEA3FF` | `0.520 0.170 295` → `#734DBE`（5.9:1） |
...
カテゴリバッジ。**カテゴリ solid 色のベタ塗り＋白文字**（記事ラベルとフィルタ選択 chip で同一スタイル）。グリフ（◆▲⬢◎■▶✦）を先頭に付ける。7 solid はいずれも白文字で WCAG AA（≥5.1:1）通過済。frontmatter 代表値はモデル solid。
