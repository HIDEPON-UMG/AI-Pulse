"""契約テスト: llm_hybrid.generate_event_extras のフォールバック挙動 4 件。

なぜ重要か（意図）:
  Gemini Free Tier クォータ天井（RPD/RPM）と「データ学習利用」を回避するため、抽出はローカル LLM
  (Qwen3.6-35B-A3B) を優先し、(a) Ollama 接続失敗 / 空応答 / schema 違反尽き、(b) GPU 占有
  検出時のみ Gemini にフォールバックする。境界 1 箇所 (llm_hybrid) で切替を locked-in するため、
  以下の class of bug を構造的に封じる:
    1. local 経由が正常成功した時は Gemini を呼ばない（クォータ温存）
    2. local の LLMError では即 Gemini にフォールバックする（dropされない）
    3. GPU 占有時は local を試さず即 Gemini を呼ぶ（ComfyUI Desktop 等の常駐とぶつからない）
    4. HYBRID_MODE=gemini_only は常時 Gemini を呼ぶ（緊急回避手段）

  feedback_check_design_principles §2 (境界 1 箇所集約) + §4 (契約テスト 1 件で不変条件 locked-in)。
"""
...
class TestHybridLocalFirst(unittest.TestCase):
    ...
    def test_gpu_busy_skips_local_calls_gemini_directly(self):
        """GPU 占有検出時は local を試さず即 Gemini を呼ぶ（ComfyUI Desktop 等の常駐回避）。"""
        with patch.object(llm_local, "generate_event_extras") as mock_local, \
             patch.object(llm_gemini, "generate_event_extras", return_value=_VALID_PAYLOAD_GEMINI) as mock_gemini, \
             patch.object(llm_hybrid, "_gpu_busy", return_value=True):
            result = llm_hybrid.generate_event_extras("本文" * 200, _META)
        self.assertEqual(result["score"], 88)
        self.assertEqual(mock_local.call_count, 0, "GPU 占有時に local を呼んではいけない")
        self.assertEqual(mock_gemini.call_count, 1)
...
class TestHybridModeOverrides(unittest.TestCase):
    ...
    def test_gemini_only_always_calls_gemini(self):
        """HYBRID_MODE=gemini_only は GPU/ローカル状態に関わらず常時 Gemini を呼ぶ。
...
class TestGpuBusyProbe(unittest.TestCase):
    """_gpu_busy の閾値判定が独立した probe で正しく動くこと。"""

    def test_above_threshold_returns_true(self):
        """probe が閾値以上の MB を返したら占有判定（即フォールバックを駆動）。"""
        self.assertTrue(llm_hybrid._gpu_busy(threshold_mb=6000, probe=lambda: 7800))

    def test_below_threshold_returns_false(self):
        """probe が閾値未満なら非占有（local を試して良い）。"""
        self.assertFalse(llm_hybrid._gpu_busy(threshold_mb=6000, probe=lambda: 1200))

    def test_probe_returns_none_means_not_busy(self):
        """nvidia-smi が見つからない / 失敗時 (None) は非占有扱い（GPU 無し環境で local を試せるように）。"""
        self.assertFalse(llm_hybrid._gpu_busy(threshold_mb=6000, probe=lambda: None))
